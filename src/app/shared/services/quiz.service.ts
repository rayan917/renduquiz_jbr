...
export class QuizService {
  ...
  getQuizContent(categoryId: any) {
    this.http.get(`http://localhost:3000/categories/${categoryId}`).subscribe((questions: any) => {
      for (const questionContent of questions.questions) {
        console.log("content",questionContent);
        this.http.get(`http://localhost:3000/answers?questionId=${questionContent.id}`).subscribe((answers: any) => {
          this.quizContent.push({
              id: questionContent.id,
              question: questionContent.questionLabel,
              answers
          });
        });
      }
